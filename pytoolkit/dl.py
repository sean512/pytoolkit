...
def pick_gpus():
    ...
    gpu_flag = map(
        lambda gpu: 0 if float(gpu['memory.used']) / float(gpu['memory.total']) < 0.5 and int(gpu['utilization.gpu']) < 50 else 1 if int(
            gpu['utilization.gpu']) >= 50 else 2 if int(gpu['utilization.gpu']) <= 50 and float(gpu['memory.used']) / float(gpu['memory.total']) < 0.7 else 3 if float(gpu['memory.used']) / float(gpu['memory.total']) < 0.7 else 4, stats)
    pairs = list(zip(ids, memory_useratios, gpu_useratios, gpu_flag))
    random.shuffle(pairs)

    bestGPUs = sorted(pairs, key=itemgetter(3, 1))
